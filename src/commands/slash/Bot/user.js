...
module.exports = {
    ...
    run: async (client, interaction) => {
      ...
      var allroles = []
      interaction.guild.members.cache.get(member.user.id).roles.cache.forEach( therole => {
        try {
      ...
      var showingroles = []
      var tew = 0
      if(!allroles[0]) showingroles.push("No roles :flushed:")
      if(!showingroles) {
        allroles.forEach(i => {
          if(tew => 7) return
          showingroles.push(allroles)
        })
      }
